...
; General
  ...
  !define MUI_ICON "chipkit.ico"

  ; NOTE: the version should be the same as the one in xboxbigbuttonwinusbdriver.inf
  !define OUTPUT_DIR		      ""
  !define PRODUCT_NAME            "chipKIT Drivers"
  !define PRODUCT_VERSION         "1.1.0.0"
  !define PRODUCT_PUBLISHER       "chipKIT"
  !define PRODUCT_WEBSITE		  "http://chipKIT.net"
  !define PRODUCT_WEBSITE_TITLE	  "chipKIT Website"
  !define PRODUCT_WEBSITE_DESC	  "Visit the chipKIT Website for more information"
  !define HARDWARE_ID             "USB\VID_0403&PID_A662"
...
; Additional info (will appear in the "details" tab of the properties window for the installer)
  ...
  VIAddVersionKey /LANG=${LANG_ENGLISH} "LegalCopyright"   "© chipKIT"
  VIAddVersionKey /LANG=${LANG_ENGLISH} "FileDescription"  "chipKIT"
...
proceed:
  ...
  File inf/stk500v2.inf
  File inf/stk500v2.cat
  File ${ARCH}\dpinst.exe  ; Used to install the driver
  File ${ARCH}\devcon.exe  ; Attempted driver uninstall, doesn't work due to some strange reason from NSIS (needs elevated cmd and suspect that is not happening)
  File uninstall_device.bat ; Used to uninstall the device from the device manager and capture the return code
  File dpinst.xml          ; Configuration for the installation of the driver
  ...
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME}" \
                   "DisplayIcon" "$\"$INSTDIR\chipkit.ico$\""
  WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\${PRODUCT_NAME}" \
